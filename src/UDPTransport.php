...
class UDPTransport implements TransportInterface
{
    ...
    public function start(): void
    {
        ...
        if ($this->socket === false) {
            throw new RuntimeException('创建UDP套接字失败: ' . socket_strerror(socket_last_error()));
        }
        ...
        if (!socket_bind($this->socket, $this->host, $this->port)) {
            ...
            throw new RuntimeException("绑定套接字失败 {$this->host}:{$this->port}: {$error}");
        }
    ...
    public function receive(): ?array
    {
        ...
        if ($bytesRead === false) {
            ...
            throw new RuntimeException('接收数据失败: ' . socket_strerror($error));
        }
